class CreateTransactionService
  ...
  def initialize(account, destination_account, amount)
    ...
    @destination_account =
      destination_account.is_a?(Account) ? destination_account : Account.find_by(id: destination_account)
    @amount = amount
  end
  ...
  def call
    ...
    if (account.id != destination_account.id) && !account.current_balance.positive?
      return OpenStruct.new(success?: false, messages: ['Insufficient funds.'])
    end
    ...
    Rails.logger.error(
      "An error occurred while creating accounts transactions from #{account.id} to #{destination_account.id}"
    )
    Rails.logger.error(e.backtrace.join("\n"))
    OpenStruct.new(
      success?: false,
      messages: ['An error occurred during transaction. Please, try again or contact support.']
    )
  end
